...
const PROJECTS = [
    {
        ...
        remoteDist: '/var/www/vamos-pos/vamos-pos-frontend/dist'
    },
    {
        name: 'Admin App',
        localDist: path.join(__dirname, 'vamos-admin-app', 'dist'),
        remoteDist: '/var/www/vamos-pos/vamos-admin-app/dist'
    },
    {
        name: 'Player App',
...
conn.on('ready', () => {
    console.log('✅ SSH Connected untuk Deploy Semua Dist!\n');
    ...
    async function deployProject(project) {
        ...
        await new Promise((resolve, reject) => {
            ...
            conn.exec(cmd, (err, stream) => {
                if (err) return reject(err);
                stream.on('data', d => {}); // Consume data to prevent hanging
    ...
    (async () => {
        try {
            for (const p of PROJECTS) {
                await deployProject(p);
            }
            console.log('🚀 Semuanya selesai! Merestart Nginx...');
