  ...
  return (
    <div className="fiery-card p-10 relative overflow-hidden group">
      ...
      <div className="space-y-4 relative z-10">
        ...
        <div className="flex items-center justify-between p-5 rounded-[28px] bg-[#101423] border border-white/5"><div className="flex items-center gap-5"><div className={`w-12 h-12 rounded-[18px] flex items-center justify-center border ${member.photo ? 'bg-primary/10 text-primary' : 'text-slate-700'}`}><User className="w-6 h-6" /></div><div><p className="text-sm font-black text-white uppercase italic">Foto Profil</p></div></div>{member.photo ? <div className="w-10 h-10 rounded-lg overflow-hidden border border-primary/30"><img src={member.photo.startsWith('http') ? member.photo : `${api.defaults.baseURL}/player/avatar-view/${member.photo.split('/').pop()}`} className="w-full h-full object-cover" /></div> : <label className="px-5 py-2 rounded-xl bg-primary/10 text-primary text-[10px] font-black uppercase italic cursor-pointer">{uploading ? 'SYNC' : 'UPLOAD'}<input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} disabled={uploading}/></label>}</div>
      </div>
    </div>
  );
}
      ...
      <div className="flex flex-col items-center">
        <div className="relative mb-5">
          <div className="w-36 h-36 rounded-[44px] bg-[#1a1f35] p-1 border-4 border-white/5 shadow-2xl overflow-hidden">
            {member.photo
              ? <img src={member.photo.startsWith('http') ? member.photo : `${api.defaults.baseURL}/player/avatar-view/${member.photo.split('/').pop()}`} alt="P" className="w-full h-full object-cover" />
