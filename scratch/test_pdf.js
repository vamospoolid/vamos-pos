...
// Create test PDF with 32-player 2-Wing bracket layout
function generate2WingBracketPDF(tournament) {
    ...
    // Wing Columns:
    // Left Wing:  Col 0 (R1 Left, x=12) -> Col 1 (R2 Left, x=58) -> Col 2 (QF Left, x=104)
    // Center:     Col 3 (SF & Final, x=127.5)
    // Right Wing: Col 4 (QF Right, x=151) -> Col 5 (R2 Right, x=197) -> Col 6 (R1 Right, x=243)
    ...
    // Calculate vertical spacing for Left (Pool A)
    const leftMatches = round1Matches.slice(0, halfR1);
    const rightMatches = round1Matches.slice(halfR1);

    const r1Spacing = availableHeight / halfR1; // ~20.5mm per match! Plenty of space!

    // Track positions for bracket connector lines
    const leftPos = {};  // `${round}_${idx}`
    const rightPos = {}; // `${round}_${idx}`
    ...
    // ─── CENTER: SEMI FINALS & GRAND FINAL ───
    // Left Semi-Final Winner spot (Top Center Left)
    const leftSF_Y = (leftPos['3_0'].cy + leftPos['3_1'].cy) / 2;
    // Right Semi-Final Winner spot (Top Center Right)
    const rightSF_Y = (rightPos['3_0'].cy + rightPos['3_1'].cy) / 2;

    // Grand Final Match Box (Center)
    const finalWidth = 46;
    ...
    // Connector Lines from QF to Center Final
    // Left QF connectors:
